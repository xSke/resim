...
class Resim:
    ...
    def handle(self, event):
        ...
        if self.ty in [
            EventType.PLAYER_SKIPPED_DUE_TO_BEING_SHELLED_FROZEN_OR_ELSEWHERE,
        ...
        if self.batter:
            print(
                "- batter mods: {} + {} ({})".format(
                    self.batter.mods, self.batting_team.mods, self.batter.name
                )
            )
        if self.pitcher:
            print(
                "- pitcher mods: {} + {} ({})".format(
                    self.pitcher.mods, self.pitching_team.mods, self.pitcher.name
                )
            )
        print(
            "- stadium mods: {} ({})".format(
                self.stadium.mods, self.stadium.data["nickname"]
            )
        )

        if self.ty == EventType.PLATE_APPEARANCE:
        ...
        if (
            self.ty
            == EventType.HIGH_PRESSURE_THE_PRESSURE_IS_ON_OFF_OVERPERFORMING_ADDED_AND_REMOVED
        ):
        ...
        if self.pitcher.has_mod("DEBT_THREE") and not self.batter.has_mod(
            "COFFEE_PERIL"
        ):
            self.roll("debt")
        ...
        if self.ty in [
            ...
            EventType.STRIKEOUT,
            EventType.STRIKE_NOT_INCLUDING_FOUL_BALLS,
        ]:
            self.handle_strike()
        ...
        elif self.ty in [EventType.HIT_SINGLE_DOUBLE_TRIPLE]:
    ...
    def handle_misc(self):
        if self.ty in [
            EventType.HOME_FIELD_ADVANTAGE_ACTIVATION,
            EventType.COFFEE_3S_GAINING_TRIPLE_THREAT,
            EventType.SOLAR_PANELS_START_UP_TEXT,
            EventType.HOMEBODY_HOMESICK_HAPPY_TO_BE_HOME,
            EventType.SUPERYUMMY_LOVES_MISSES_PEANUTS,
            EventType.PERK,
        ...
        if self.ty in [
            ...
            EventType.UNDERSEA,
            EventType.ADDED_MODIFICATION_DUE_TO_ANOTHER_MOD_UNDER_OVER_PERK_PSYCHO_ACOUSTICS_ETC,
            EventType.REMOVED_MODIFICATION_ADDED_DUE_TO_ANOTHER_MOD_UNDER_OVER_PERK,
            EventType.CHANGED_MODIFIER_DUE_TO_ANOTHER_MODIFIER,
            EventType.REMOVED_MULTIPLE_MODIFICATIONS_ADDED_DUE_TO_ANOTHER_MOD_ECHO,
            EventType.ADDED_MULTIPLE_MODIFICATIONS_DUE_TO_ANOTHER_MOD_ECHO,
        ]:
            # skipping mod added/removed
            return True
        if self.ty in [
            EventType.BLACK_HOLE_COLLECT_10_IN_GAME,
            EventType.SUN_2_COLLECT_10_IN_GAME,
            EventType.SUN_2_GRANTS_A_WIN_OUTCOME_PRE_S20,
            EventType.BLACK_HOLE_SWALLOWS_A_WIN_OUTCOME_PRE_S20,
        ]:
        ...
        if self.ty in [
            ...
            return True
        if self.ty in [
            EventType.PLAYER_INCINERATION_REPLACEMENT,
            EventType.PLAYER_TEAM_ENTERS_THE_HALL_OF_FLAME,
            EventType.PLAYER_HATCHED_REPLICA_CREATED,
        ]:
            # skipping incineration stuff
            return True
        if self.ty in [
            EventType.PLAYER_REMOVED_FROM_TEAM,
            EventType.MODIFICATION_CHANGE_WIRED_TIRED_TRANSITIONS_REROLL_EGO_UPGRADE_ETC,
        ]:
            # skipping echo/static
            return True
        if (
            self.ty == EventType.SOLAR_ECLIPSE_INCINERATION
            and "parent" in self.event["metadata"]
        ):
        ...
        if self.ty == EventType.PITCHER_CHANGE:
            ...
            return True
        if self.ty in [
            EventType.REMOVED_MODIFICATION_SAME_AS_ABOVE_BUT_WHEN_THEY_DISAPPEAR,
            EventType.PLAYER_MOVE_RECRUIT_ETC,
            EventType.INVESTIGATION_PROGRESS_UPSHELLABLE_FOR_FISH,
        ]:
            # skip liquid/plasma plot nonsense
            return True
        if self.ty == EventType.REVERB_SHUFFLE_ROTATION:
            # skip reverb
            return True
        if self.ty == EventType.PLAYER_TRADE:
            # skip feedback
            return True
        if self.ty in [EventType.END_OF_INNING]:
            ...
            if self.update["inning"] == 2:
                ...
                print(
                    "home pitcher mods: {} ({})".format(
                        self.home_pitcher.data["permAttr"], self.home_pitcher.name
                    )
                )
                print(
                    "away pitcher mods: {} ({})".format(
                        self.away_pitcher.data["permAttr"], self.away_pitcher.name
                    )
                )
                if (
        ...
        if self.ty in [EventType.HALF_INNING]:
            ...
            return True
        if self.ty == EventType.SALMON_INNING_BEGINS_AGAIN:
            self.roll("salmon")
            ...
            return True
        if self.ty in [
            EventType.GAME_END_LOG,
            EventType.ADDED_MODIFICATION_TRIPLE_THREAT_FREE_REFILL_MAGMATIC_INHABITING_ETC,
            EventType.REMOVED_MODIFICATION_SAME_AS_ABOVE_BUT_WHEN_THEY_DISAPPEAR,
            EventType.TIMED_MODIFICATION_EXPIRES,
            EventType.FINAL_STANDINGS,
            EventType.TEAM_SHAMED_BY_ANOTHER,
            EventType.TEAM_SHAMES_ANOTHER,
            EventType.ELIMINATED_FROM_POSTSEASON,
            EventType.POSTSEASON_ADVANCE,
        ]:
            # skipping game end

            if self.ty == EventType.GAME_END_LOG and self.weather.is_coffee():
    ...
    def handle_bird_ambush(self):
        if self.weather == Weather.BIRDS:
            ...
            if self.strikes == 0:
                ...
                if self.ty == EventType.BIRDS_FRIEND_OF_CROWS_PROC:
                    self.handle_batter_reverb()  # i guess???
                    return True

    def handle_charm(self):
        pitch_charm_eligible = (
            self.update["atBatBalls"] == 0 and self.update["atBatStrikes"] == 0
        )
        batter_charm_eligible = (
            self.batting_team.has_mod("LOVE") and pitch_charm_eligible
        )
        pitcher_charm_eligible = (
            self.pitching_team.has_mod("LOVE") and pitch_charm_eligible
        )
    ...
    def handle_electric(self):
        ...
        if self.batting_team.has_mod("ELECTRIC") and self.update["atBatStrikes"] > 0:
            ...
            if self.ty == EventType.STRIKE_ZAPPED_BY_ELECTRIC_BLOOD:
                # successful zap!
                return True
    ...
    def handle_batter_reverb(self):
        if self.batter and self.batter.has_mod("REVERBERATING"):
            is_at_bat_end = self.ty in [
                ...
                EventType.FLYOUT,
                EventType.GROUND_OUT,
                EventType.BIRDS_FRIEND_OF_CROWS_PROC,
            ]  # ambush i guess
            ...
            if self.ty in [
                EventType.HOME_RUN,
                EventType.HIT_SINGLE_DOUBLE_TRIPLE,
            ]:
    ...
    def handle_strike(self):
        if ", swinging" in self.desc or "strikes out swinging." in self.desc:
            ...
            swing_roll = self.roll("swing")
            self.log_roll(
                self.swing_on_strike_rolls
                if self.is_strike
                else self.swing_on_ball_rolls,
                "StrikeSwinging",
                swing_roll,
                True,
            )
            ...
            if not self.is_flinching():
                swing_roll = self.roll("swing")
                self.log_roll(
                    self.swing_on_strike_rolls, "StrikeLooking", swing_roll, False
                )
    ...
    def try_roll_salmon(self):
        ...
        if (
            ...
            if (
                current_away_score != last_inning_away_score
                or current_home_score != last_inning_home_score
            ):
                self.roll("salmon")
    ...
    def handle_out(self):
        ...
        fielder = None
        if self.ty == EventType.FLYOUT:  # flyout
        ...
        is_fc_dp = (
            "into a double play!" in self.desc
            or "reaches on fielder's choice" in self.desc
        )
        if (
            not is_fc_dp
            and self.batter.has_mod("DEBT_THREE")
            and fielder
            and not fielder.has_mod("COFFEE_PERIL")
        ):
            self.roll("debt")
    ...
    def handle_hit_advances(self, bases_hit):
        ...
        bases_after = make_base_map(self.next_update)
        for runner_id, base, roll_outcome in calculate_advances(
            bases_before, bases_after, bases_hit
        ):
            self.roll("adv ({}, {})".format(base, roll_outcome))
    ...
    def roll_foul(self, known_outcome: bool):
        ...
        musc = (
            self.batter.data["musclitude"]
            * self.get_batter_multiplier(relevant_attr="musclitude")
            * (1 + 0.2 * vibes)
        )
        thwack = (
        ...
        div = (
            self.batter.data["divinity"]
            * self.get_batter_multiplier(relevant_attr="divinity")
            * (1 + 0.2 * vibes)
        )
        foul_threshold = (
            0.25
        ...
        is_0_no_eligible = (
            self.batting_team.has_mod("O_NO") and self.strikes == 2 and self.balls == 0
        )
        if not is_0_no_eligible:
            if known_outcome and foul_roll > foul_threshold:
                print(
                    "!!! too high foul roll ({} > {})".format(foul_roll, foul_threshold)
                )
            elif not known_outcome and foul_roll < foul_threshold:
                print(
                    "!!! too low foul roll ({} < {})".format(foul_roll, foul_threshold)
                )
    ...
    def handle_weather(self):
        if self.weather == Weather.SUN_2:
            ...
            if self.ty == EventType.SOLAR_ECLIPSE_INCINERATION:
                self.roll("target")
                # self.roll("target")
            ...
            for player_id in fire_eater_eligible:
                ...
                if player.has_mod("FIRE_EATER") and not player.has_mod("ELSEWHERE"):
                    ...
                    if (
                        self.ty
                        == EventType.SOLAR_ECLIPSE_INCINERATION_BLOCKED_FIREPROOF_FIRE_EATER
                    ):
            ...
            if self.ty == EventType.BLOODDRAIN_SIPHON_PROC:
            ...
            if self.ty == EventType.PEANUTS_FLAVOR_TEXT:
                self.roll("peanut message")
                return True

            self.roll("peanuts")
            if self.ty == EventType.PEANUTS_ALLERGIC_REACTION:
                self.roll("target")
                return True
            ...
            if self.ty == EventType.TASTING_THE_INFINITE_SHELLING_VIA_HONEY_ROASTED:
            ...
            if self.ty == EventType.BIRDS_CIRCLING_NO_SHELLED_PLAYER:
                # the birds circle...
                return True
            ...
            if self.weather.can_echo() and (
                ...
                if self.ty == EventType.ECHO_MESSAGE:
                    ...
                    return True
                if self.ty in [
                    EventType.SUPERPOSITION_ECHO_INTO_STATIC,
                    EventType.RECEIVER_BECOMES_AN_ECHO,
                ]:
                    self.roll("echo target")
                    self.roll("echo target")
                    return True
            ...
            self.roll("coffee")
            if self.ty == EventType.COFFEE_GAINING_LOSING_WIRED_TIRED:
            ...
            if self.ty == EventType.COFFEE_2_GAINING_A_FREE_REFILL:
    ...
    def handle_flooding(self):
        if self.weather == Weather.FLOODING:
            ...
            if self.ty == EventType.FLOODING_BASERUNNERS_SWEPT:
                # handle flood
    ...
    def handle_consumers(self):
        ...
        for team in teams:
            ...
            if level >= 5:
                ...
                if self.ty == EventType.CONSUMERS_ATTACK_INCL_DEFENSE_WITH_ITEMS_YOLKED:
                    ...
                    if is_on_team:
                        ...
                        if attacked_player.data["soul"] == 1:
                            ...
                            print(
                                "!!! {} lost last soul, redacting".format(
    ...
    def handle_secret_base(self):
        ...
        did_attractor_enter_this_tick = (
            ...
            and self.ty != EventType.ENTERING_THE_SECRET_BASE
        )
        ...
        if secret_base_exit_eligible:
            ...
            if self.ty == EventType.EXITING_THE_SECRET_BASE:
                return True

        if secret_base_enter_eligible:
            self.roll("secret base enter")
            if self.ty == EventType.ENTERING_THE_SECRET_BASE:
                return True
    ...
    def throw_pitch(self, known_result=None):
        ...
        threshold = (
            constant + ruth_factor * (ruth * (1 + 0.2 * vibes)) + 0.2 * fwd + 0.1 * musc
        )
    ...
    def setup_data(self, event):
        ...
        batter_id = (
            update["awayBatter"] if update["topOfInning"] else update["homeBatter"]
        )
        if not batter_id and next_update:
            batter_id = (
                next_update["awayBatter"]
                if next_update["topOfInning"]
                else next_update["homeBatter"]
            )
        pitcher_id = (
            update["homePitcher"] if update["topOfInning"] else update["awayPitcher"]
        )
        if not pitcher_id and next_update:
            pitcher_id = (
                next_update["homePitcher"]
                if next_update["topOfInning"]
                else next_update["awayPitcher"]
            )
        ...
        self.home_pitcher = (
            self.data.get_player(home_pitcher_id) if home_pitcher_id else None
        )
        self.away_pitcher = (
            self.data.get_player(away_pitcher_id) if away_pitcher_id else None
        )

        self.stadium = (
            self.data.get_stadium(update["stadiumId"])
            if update["stadiumId"]
            else null_stadium
        )
        ...
        self.max_outs = (
            update["awayOuts"] if update["topOfInning"] else update["homeOuts"]
        )
        self.strikes = update["atBatStrikes"]
        self.max_strikes = (
            update["awayStrikes"] if update["topOfInning"] else update["homeStrikes"]
        )
        self.balls = update["atBatBalls"]
        self.max_balls = (
            update["awayBalls"] if update["topOfInning"] else update["homeBalls"]
        )
    ...
    def apply_event_changes(self, event):
        ...
        if event["type"] in [
            ...
            if event["playerTags"]:
                ...
                if meta["mod"] not in team.data[position]:
                    print(
                        "!!! warn: trying to remove mod {} but can't find it".format(
                            meta["mod"]
                        )
                    )
                else:
        ...
        # mod replaced
        if event["type"] in [EventType.CHANGED_MODIFIER_DUE_TO_ANOTHER_MODIFIER]:
            position = mod_positions[meta["type"]]
        ...
        # timed mods wore off
        if event["type"] in [EventType.TIMED_MODIFICATION_EXPIRES]:
            position = mod_positions[meta["type"]]
        ...
        # roster swap
        if event["type"] == EventType.PLAYER_TRADE:
    ...
    def get_batter_multiplier(self, relevant_batter=None, relevant_attr=None):
        ...
        for mod in itertools.chain(batter.mods, self.batting_team.mods):
            if mod == "OVERPERFORMING":
                ...
                roster_size = len(self.batting_team.data["lineup"]) + len(
                    self.batting_team.data["rotation"]
                )
    ...
    def get_pitcher_multiplier(self, relevant_attr=None):
        ...
        for mod in itertools.chain(self.pitcher.mods, self.pitching_team.mods):
            if mod == "OVERPERFORMING":
                ...
                roster_size = len(self.pitching_team.data["lineup"]) + len(
                    self.pitching_team.data["rotation"]
                )
